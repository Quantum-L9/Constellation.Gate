...
timeout, status mapping, response decoding) decide *how*. This module is the
only place in Gate production code allowed to own the "how" for a worker
packet, and ``tests/architecture/test_worker_transport_seam.py`` fails the
...
The typed errors below are the second reason this is a module rather than an
inline block: flattening every failure into ``RuntimeError`` loses the
distinction between "the worker is down" (mark it unhealthy), "the worker did
not answer in time" (say nothing about its health), and "the worker answered
with something unusable" (an upstream bug, not a Gate one).
"""
