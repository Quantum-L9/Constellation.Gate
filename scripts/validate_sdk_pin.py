...
``v1`` — not a commit sha. Gate declares that channel in two places, and for as
long as both were pinned by hand they drifted: ``pyproject.toml`` named
69c6c67 while ``.pre-commit-config.yaml`` named 0d50f64, so the mypy hook typed
against a different SDK than CI installed.
...
--verify-tag   Resolves ``v1`` at the canonical remote and requires the lock's
               ...
               resolved — a lock that cannot be checked is not a lock that passed.
"""
...
# Active surfaces. A missing one is a FAIL, not a skip: a surface that stops
# being enumerated is exactly how the pre-commit sha drifted unnoticed.
MANIFESTS = (
    "constellation-gate/pyproject.toml",
    "constellation-gate/.pre-commit-config.yaml",
)
LOCK = "constellation-gate/requirements.lock"

SHA_RE = re.compile(r"\b[0-9a-f]{40}\b")
# The ref a manifest declares after Gate_SDK.git@ — tag, branch, or sha.
MANIFEST_REF_RE = re.compile(r"Gate_SDK(?:\.git)?@([0-9A-Za-z._/-]+)")
# The generated lock rewrites the git dependency to a hash-verified archive.
...
def _git(*args: str) -> str:
    # Absolute git: controller worktrees put a guard ahead of PATH.
...
def resolve_remote_tag(remote: str, tag: str) -> str | None:
    ...
    try:
        output = _git(
            "ls-remote", "--tags", "--end-of-options", safe_remote(remote), f"refs/tags/{tag}"
        )
    except RuntimeError:
        return None
...
def check_manifest(rel: str, text: str) -> list[str]:
    """A manifest declares the compatibility channel and nothing else."""
    errors: list[str] = []
...
def check_lock(rel: str, text: str) -> list[str]:
    ...
    if lock_resolution(text) is None:
        return [
            ...
            "40-character object — regenerate with "
            "constellation-gate/scripts/lock_requirements.sh"
        ]
    return []
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"PASS Gate declares {CANONICAL_REPO}@{MAJOR_TAG} ({mode})")
    return 0
